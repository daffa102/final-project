...
class _MainWrapperState extends State<MainWrapper> {
  ...
  Widget _buildFloatingBottomBar(int selectedIndex, ThemeData theme, bool isDark) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: 20.w, vertical: 12.h),
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF1E2938).withValues(alpha: 0.9) : Colors.white.withValues(alpha: 0.95),
      ...
      child: Row(
        ...
        children: [
          _buildNavItem(0, Icons.description_outlined, Icons.description_rounded, selectedIndex),
          _buildNavItem(1, Icons.account_balance_wallet_outlined, Icons.account_balance_wallet_rounded, selectedIndex),
          _buildNavItem(2, Icons.grid_view_outlined, Icons.grid_view_rounded, selectedIndex),
          _buildNavItem(3, Icons.notifications_outlined, Icons.notifications_rounded, selectedIndex),
          _buildNavItem(4, Icons.person_outline, Icons.person_rounded, selectedIndex),
        ],
      ),
    );
  }

  Widget _buildNavItem(int index, IconData icon, IconData activeIcon, int selectedIndex) {
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  Text('Aktivitas', style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontSize: 18.sp, fontWeight: FontWeight.w600, fontFamily: 'Poppins')),
                  SizedBox(width: 8.w),
                  GestureDetector(
                    onTap: () => context.read<NavigationProvider>().setIndex(0),
                    child: Text('Lihat Semua', style: TextStyle(color: const Color(0xFFBEF364), fontSize: 12.sp, fontWeight: FontWeight.w500)),
                  ),
                  const Spacer(),
                  GestureDetector(
                  ...
                  if (activities.isEmpty) {
                    return Center(
                      child: Column(
                        ...
                        children: [
                          Icon(Icons.notifications_none_rounded, size: 64.r, color: isDark ? Colors.white10 : Colors.black12),
                          SizedBox(height: 16.h),
                          Text('Belum ada aktivitas', style: TextStyle(color: isDark ? Colors.white24 : Colors.black26, fontSize: 14.sp)),
                        ],
                      ),
                    );
                  }
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final amount = NumberFormat.currency(locale: 'id_ID', symbol: 'Rp', decimalDigits: 0)
                          .format(double.tryParse(trx['total_amount'].toString()) ?? 0);
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(color: const Color(0xFFBEF364).withValues(alpha: 0.15), shape: BoxShape.circle),
                              child: Icon(Icons.shopping_bag_outlined, color: const Color(0xFFBEF364), size: 20.r),
                            ),
                            SizedBox(width: 16.w),
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text('Transaksi Berhasil', style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontSize: 14.sp, fontWeight: FontWeight.w600)),
                                  Text('${trx['invoice_number']} • $amount', style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 12.sp)),
                                ],
                              ),
                            ),
                            Text(
